...
detect_packet <- function(input, packet = character(0), characters_processed = 0, n_char = 4) {
  ...
  if (length(packet) == n_char & anyDuplicated(packet) == 0) {
  ...
  #update packet
  if (length(packet) < n_char){
    new_packet <- c(packet, new_char)} else {
      new_packet <- c(packet[-1], new_char)
    }

  #recursion
  detect_packet(input, new_packet, characters_processed, n_char)
...
#detect_packet(input, n_char = 14)
#Error: C stack usage  15924544 is too close to the limit

detect_packet2(input, n_char = 14)
